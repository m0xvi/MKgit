...
rup_worker_apply() {
    ...
    if [ -n "$cf" ] && [ -n "$uf" ] && [ "$cf" != "$uf" ] && ! rup_is_newer "$uf" "$cf" && [ "$uf" != "-" ] && [ "$cf" != "-" ]; then
        rup_send "ℹ️ <b>$name</b>: доступно обновление RouterBOOT firmware ($cf → $uf). Примените при необходимости: <code>/system routerboard upgrade</code> + reboot."
    fi
